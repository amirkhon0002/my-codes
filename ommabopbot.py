...
from ibuttons import inline_menu, coursesMenu, fullstackMenu, about_django, newMenu
TOKEN = "5167230229:AAHCIpLeFT2tAftPS_TDNgoXtMwfg8wwAmI"
from aiogram.dispatcher.filters import Text
from new_call import course_callback
...
async def get_ibutton(message:types.Message):
    await message.answer_photo(photo="https://www.google.com/imgres?imgurl=http%3A%2F%2Fenablementnepal.org%2Fwp-content%2Fuploads%2F2020%2F02%2Fcourses.png&imgrefurl=http%3A%2F%2Fenablementnepal.org%2Fcourse%2Fmanagement-and-leadership-in-cbr-and-did%2F&tbnid=TItrefqOh15bIM&vet=12ahUKEwj0naWK_az2AhVc7rsIHSUaAkgQMygHegUIARC2AQ..i&docid=MumrPg_Ic9rkxM&w=750&h=451&q=course&ved=2ahUKEwj0naWK_az2AhVc7rsIHSUaAkgQMygHegUIARC2AQ",
                               caption="Choose and take information about our courses ⬇️ ", reply_markup=inline_menu
                               )
...
async def get_django(call: types.CallbackQuery):
    ...
    logging.info(f"{callback_data=}")
    await call.message.delete()
    await call.message.answer("You choose Django section,\nClick here and you can take more information\nabout django.\n🔽🔽🔽🔽🔽🔽🔽🔽🔽",
                              reply_markup=about_django)


@dp.callback_query_handler(text="telegrambot")
async def get_telegram(call: types.CallbackQuery):
    callback_data =call.data
    logging.info(f"{callback_data=}")
    await call.answer("I like telegram bot", cache_time=60, show_alert=True)

@dp.callback_query_handler(course_callback.filter(item_name="Html"))
async def buying_course(call: types.CallbackQuery, callback_data: dict):
    course = callback_data.get("item_name")
    logging.info(f"{course=}")
    logging.info(f"{callback_data=}")
    await call.message.answer(f"You choose html")
